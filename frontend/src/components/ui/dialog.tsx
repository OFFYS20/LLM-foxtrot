...
>(({ className, children, wide, ...props }, ref) => (
  <DialogPrimitive.Portal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed left-1/2 top-1/2 z-50 w-[95vw] -translate-x-1/2 -translate-y-1/2 rounded-lg border border-border bg-panel shadow-2xl",
    ...
    >
      {children}
      <DialogPrimitive.Close className="absolute right-3 top-3 rounded-sm p-1 text-ink-muted transition-colors hover:bg-elevated hover:text-ink focus:outline-none">
        <X className="h-4 w-4" />
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Description ref={ref} className={cn("mt-1 text-xs text-ink-muted", className)} {...props} />
));
